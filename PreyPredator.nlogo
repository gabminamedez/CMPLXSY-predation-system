...
to go
  ...
  ask predators [
;    ifelse one-of preys in-radius 1 != nobody [
;      move-to one-of preys in-radius 1
;    ]
;    [
;      ifelse coin-flip?
;      [ right random max-move ]
...
;    ]
    ...
    ifelse one-of preys in-radius 0.5 != nobody
      [ ask one-of preys in-radius 0.5 [ die ]
        set energy (energy + 5)
        ]
      [ set energy (energy - 1)]
    if randomize < reproduction-chance [
  ...
  ask patches [
    if pcolor = black [ set growth-time (growth-time - 1) ]
    if growth-time <= 0 [
      set pcolor green
      set growth-time food-growth
    ]
...
"" ""
PENS
"feeds" 1.0 0 -10899396 true "" "plot count patches with [pcolor = green]"
